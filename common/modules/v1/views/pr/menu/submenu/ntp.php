        ...
        <?php if($bid->winners){ ?>
            ...
            <?php foreach($bid->winners as $winner){ ?>
                <tr>
                    ...
                    <td style="width: 5%;" align=right><?= $model->getSupplierHasNtps($winner->id) ? '<i class="fa fa-check text-green"></i>' : '' ?></td>
                </tr>
                <?php if($winner->getPos($bid->id)){ ?>
                    <?php foreach($winner->getPos($bid->id) as $po){ ?>
                        <tr onclick="createNtp('<?= $model->id ?>','<?= $po->id ?>','<?= $j ?>','<?= $i ?>','<?= $k ?>');">
                            <td style="width: 10%;">&nbsp;</td>
                            <td style="width: 85%;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);">NTP for <?= $po->type == 'PO' ? 'PO No.' : 'Contract No.' ?> <?= $po->pocnNo ?></a></td>
                            <td style="width: 5%;" align=right><?= $model->getSupplierHasNtp($po->id) ? '<i class="fa fa-check text-green"></i>' : '' ?></td>
                        </tr>
                        <?php $k++ ?>
                    <?php } ?>
                <?php } ?>
...
<?php
    $script = '
        function createNtp(id, po_id, j, i, k)
        {
            $.ajax({
                url: "'.Url::to(['/v1/pr/create-ntp']).'?id=" + id + "&po_id=" + po_id + "&j=" + j + "&i=" + i + "&k=" + k,
                beforeSend: function(){
