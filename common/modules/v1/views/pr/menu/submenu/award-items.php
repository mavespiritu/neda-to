...
<h3 class="panel-title">6. NOA</h3>
<br>
<p><i class="fa fa-exclamation-circle"></i> Award items by generating NOA report.</p>
    ...
    <?php foreach($pos as $po){ ?>
        <li class="item">
            ...
            <div class="product-info">
                <a href="javascript:void(0)" onclick="createNoa('<?= $model->id ?>','<?= $po->id ?>','<?= $i ?>');" class="product-title"><?= $po->type == 'PO' ? 'PO No. '.$po->pocnNo : 'Contract No. '.$po->pocnNo ?>
                <?= $po->noas ? '<span class="badge bg-green pull-right"><i class="fa fa-check"></i></span>' : '' ?>
                </a>
...
<?php
    $script = '
        function awardItems(id, po_id, i)
        {
            $.ajax({
                url: "'.Url::to(['/v1/pr/award-item']).'?id=" + id + "&po_id=" + po_id + "&i=" + i,
                beforeSend: function(){
